...
class _CarRentalHomeScreenState extends State<CarRentalHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_serviceProvider == null || isLoadingPage) {
        ...
        children: [
          _buildSwitchRow(
            ...
            onChanged: (newValue) async {
              if (_officeDetailId == null || _officeService == null) return;
              setState(() => isUpdatingDelivery = true);
              try {
                final success = await _officeService!.updateAvailability(
      ...
      builder: (context, provider, child) {
        ...
        switch (selectedTab) {
          case 0:
            ...
            break;
          case 1:
            currentList = provider.inProgressRequests;
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (selectedTab == 0)
            _buildActionButtonForCard(title: "قبول الطلب", price: request.requestData.price, color: Colors.green, isLoading: _processingRequestId == request.id, onPressed: () => _acceptRequest(request)),
            
          if (selectedTab == 1)
            _buildActionButtonForCard(title: "إنهاء الطلب", price: request.requestData.price, color: Colors.red, isLoading: _processingRequestId == request.id, onPressed: () => _completeRequest(request)),
  ...
  Widget _buildActionButtonForCard({required String title, int? price, required Color color, required bool isLoading, required VoidCallback onPressed}) {
    return SizedBox(
      width: double.infinity,
      child: ElevatedButton(
        onPressed: isLoading ? null : onPressed,
        style: ElevatedButton.styleFrom(backgroundColor: color, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), elevation: 1),
        child: isLoading
